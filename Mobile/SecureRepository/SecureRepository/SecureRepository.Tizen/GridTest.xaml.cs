...
namespace SecureRepository.Tizen
{
    ...
    public partial class MainPage : ContentPage
    {
        ...
        protected override void OnSizeAllocated(double width, double height)
        {
            ...
            listView.Scale = 1; // Reset listView to 1.0 Scale (TV requires to scale it to 1.05) 
            if (width != this.width || height != this.height) // width or height has changed
            {
                ...
                if (height < 500 && width < 500) //Wearable UI, it`s much simplified
                    //(Screen height may not equals width so It only checks pixel count) 
                {
                    ...
                    MainGrid.Children.Add(listView, 0, 0);
                    MainGrid.Children.Add(appInfo, 0, 1);
                    appInfo.HorizontalOptions = LayoutOptions.Center;
                    OnAddKeyClicked(null, null); //User can only see already added items
                }
                else if (width > height) //Realigns UI for vertical mode
                {
                    ...
                    if (width == 1920 & height == 1080) // fix Listview size on TV emulator
                        listView.Scale = 1.05;
                }
